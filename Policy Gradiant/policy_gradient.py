...
class PolicyGradient(object):
    ...
    def __init__(self,n_actions,n_features,
        ...
        self.ep_obs, self.ep_as, self.ep_rs = [], [], []  # 这是我们存储回合信息的 list
    ...
    def _build_net(self):
        ...
        with tf.name_scope('inputs'):
            self.tf_obs = tf.placeholder(tf.float32, [None, self.features],
        ...
        with tf.name_scope('loss'):
            ...
            # 使用负对数似然函数，即交叉熵函数
            neg_log_prob = \
                tf.nn.sparse_softmax_cross_entropy_with_logits(logits=all_act,
    ...
    def choose_action(self, observation):
        ...
        return action
    # 就是将这一步的 observation, action, reward 加到列表中去.
    # 因为本回合完毕之后要清空列表, 然后存储下一回合的数据,
    # 所以我们会在 learn() 当中进行清空列表的动作.
    ...
    #### 模拟一条完整的序列，并记录下每个状态的v
    def _discount_and_norm_rewards(self):
        # discount episode rewards
        discounted_ep_rs = np.zeros_like(self.ep_rs)
        running_add = 0

        # ep_rs记录的是每个状态的奖励r，而不是状态的收获，所以d_e_r记录的是v(s)
        # 注意reversed
        for t in reversed(range(0, len(self.ep_rs))):
            running_add = running_add * self.gamma + self.ep_rs[t]
            discounted_ep_rs[t] = running_add

        # normalize episode rewards,为了计算方便
